...
class ChannelAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not profile_id or not participant_id:
            return Response({'error': 'profile_id and participant_id are required'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        participant_1 = ChannelParticipants.objects.filter(channel=channel, profile=profile).first()
        participant_2 = ChannelParticipants.objects.filter(channel=channel, profile=participant).first()
